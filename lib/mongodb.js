...
MongoDB.prototype.fromDatabase = function (model, data) {
  ...
  for (var p in props) {
    ...
    } else if(data[p] && prop.type === GeoPoint) {
...
MongoDB.prototype.all = function all(model, filter, options, callback) {
  ...
  function processResponse(err, cursor) {
    ...
    var order = {};
    // don't apply sorting if dealing with a geo query without maxDistance set
    if (shouldSort(filter.where)) {
      if (!filter.order) {
    ...
    async.each(models, function (model, modelCallback) {
      ...
      for (var p in properties) {
        ...
        if (properties[p].index) {
          ...
          else if (properties[p].type.name === 'GeoPoint') {
            var indexType =
...
MongoDB.prototype.buildNearFilter = function buildNearFilter(query, near) {

  near.near = GeoPoint(near.near);

  query.where[near.key] = {
    near: {
      $geometry: {
        coordinates: [near.near.lng, near.near.lat],
        type: 'Point'
      }
    }
  };
  ...
  if (near.maxDistance) {
    query.where[near.key].near.$maxDistance = near.maxDistance * 1000; // query is specified in km
  }
};

function shouldSort(where) {

  if (!where) return true;

  for (var k in where) {
    if (where[k] && typeof where[k] === 'object' && where[k].near && !where[k].near.$maxDistance) {
      return false;
    }
  }
